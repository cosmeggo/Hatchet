...
extern MY_HIGHP_OR_MEDIUMP vec2 divine;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float cy = uv_scaled_centered.y * 95/71 * 0.3;


    float s1 = sin(2 * cy) * cos(t * 0.6);
    float s2 = sin(cy + 2 * cx);
    float s3 = sin(cy - 2 * cx);

    float grid1 = pow(1.2 * max(0, (max(max(s1,s2),s3) / (pow(s1 + s2 + s3, 2) + 1)) - 0.4), 0.5);
    ...
    hsl.z = min(hsl.z + 0.3, 1);
    
    hsl.y = max(hsl.y, 0.3);
    hsl.y += grid1/2;
    hsl.y += (1 - grid1)/8;

    hsl.y = hsl.y * 0.6 + osc * 0.3 + 0.1;

    hsl.x = (hsl.x + 10 * floor(grid1 + 1) * (0.95 + sw1/10))/(1 + 10 * floor(grid1 + 1));
    hsl.x += cy/170 - 0.3;
    ...
    tex.rgb = tex.rgb * 0.5 + HSVtoRGB(hsl).rgb * 0.5;
